...
struct Solver {
    ...
    [[nodiscard]] std::vector<Optimization> generateOptimizations(const State &state) const {
        ...
        for (int i = 0; i < noMachines; i++) {
            ...
            bool canReduceGlobal = true;
            double totalLoad = 0.0;

            for (int j = 0; j < noWeeks; j++) {
                totalLoad += machine.loads[j];

                if (machine.weekDayPatterns[j] != machine.weekDayPatterns[0] || machine.weekDayPatterns[j] == 1) {
                    canReduceGlobal = false;
                    break;
                }

                if (machine.weekEndPatterns[j] != machine.weekEndPatterns[0] || machine.weekEndPatterns[j] == 1) {
                    canReduceGlobal = false;
                    break;
                }
            }

            if (noInteractions != 300 && (totalLoad / ((double) noWeeks)) > 0.7) {
                canReduceGlobal = false;
            }
