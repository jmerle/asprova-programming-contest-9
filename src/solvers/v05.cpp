...
struct Solver {
    ...
    [[nodiscard]] std::vector<Optimization> generateOptimizations(const State &state) const {
        ...
        if (state.score != 0) {
            bool hasApplicableOptimizations = false;
            for (const auto &optimization : optimizations) {
                if (badOptimizations.find(optimization.id) == badOptimizations.end()) {
                    hasApplicableOptimizations = true;
                    break;
                }
            }

            if (!hasApplicableOptimizations) {
                for (int i = 0; i < noMachines; i++) {
                    auto &machine = state.machines[i];
                    ...
                    if (lastOperatingWeekDay != -1) {
                        ...
                        bool useOptimization = splitWeek != -1 || remainingChanges >= 1;
                        if (splitWeek == -1) {
